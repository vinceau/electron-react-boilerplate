...
export type MainHandler<I extends Payload, O extends Payload> = (
  params: I
) => Promise<O>;
...
export const makeEndpoint: EndpointMaker = {
  main: <I extends Payload, O extends Payload>(
    ...
    if (process.type === 'worker') {
      ...
      log.error(
        'Attempt to register duplicate endpoint with name',
        name,
        process.type
      );
      ...
      return {
        renderer: {
          trigger: async (payload: I): Promise<MainEndpointResponse<O>> => {
            const result: MainEndpointResponse<O> = await ipcRenderer.invoke(
              name,
              payload
            );
            return result;
